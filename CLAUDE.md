...
src/
  ...
  i18n/           index.ts (Proxy reativo `t`; dicionários pt.ts/en.ts/es.ts — 518 chaves; `setLangDict`+`LANGS`; `LanguageSelect` em Sidebar/LoginPage — no `TopNav` o idioma virou um botão cíclico dentro do menu de perfil, ver seção de nuvem)
...
- **1013 testes verdes (85 arquivos)**, build verde, smoke verde (Chromium headless: render, navegação, drill, painel coach).
