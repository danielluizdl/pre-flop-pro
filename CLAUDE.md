...
- **LoginPage** (login/signup/forgot + widget `Turnstile.tsx` se `VITE_TURNSTILE_KEY`), **WelcomeModal**, **OnboardingTour** (07/06, `src/components/Auth/OnboardingTour.tsx`), **ChangePasswordModal**. Ao fechar o `WelcomeModal`, em vez de só zerar `currentUser.firstLogin` (o que deixava o overlay montado-porém-invisível — `fixed inset-0` sem `justSignedUp` virar `false` mantinha o componente na árvore), agora zera `justSignedUp` **e** seta `onboardingStep: 0` no store, disparando o tour guiado só para quem acabou de criar conta.
  - **`OnboardingTour`, versão elaborada (07/06)**: navega de verdade por **8 passos em 6 páginas reais** (Dashboard → Meus Ranges → Configurar Mesa → Editor → Configurar Mesa/cenários → Drill → Range Check → Histórico) chamando `setPage(...)` (e, nos passos de criar range, `setupNewRange(8,true,0.5)`/`initTableConfig()` — os mesmos actions que os botões reais "Continuar"/"Avançar" chamam, então é navegação honesta, não encenada). Cada passo destaca um elemento real da página via atributo **`data-tour="<chave>"`** (`dashboard-hero`, `ranges-new`, `setup-tablesize`, `editor-matrix`, `table-editor-table`, `drill-select`, `exercise-select`, `stats-header`, espalhados em `Dashboard.tsx`/`SituationsPage.tsx`/`RangeSetupPage.tsx`/`RangeEditorPage.tsx`/`TableEditorPage.tsx`/`TrainerPage.tsx`/`ExercisePage.tsx`/`StatsPage.tsx`), medido via `getBoundingClientRect` com **polling a cada 100ms por até 2.5s** (as páginas são lazy/Suspense — o alvo só existe depois do chunk carregar) — se não achar a tempo, cai num **fallback sem spotlight** (painel centralizado, tour não trava). O painel (título/descrição/contador "N/total") é **sempre visível e interativo desde o primeiro frame** (não espera medir pra aparecer) — só a posição anima do centro pro alvo quando medido; assim "Pular tutorial" fica sempre clicável. Nota fixa no rodapé do painel: *"Você pode reabrir este tutorial quando quiser em Perfil → Rever tutorial."* z-index 60 (acima do `TopNav`/modais, que usam 50). **Risco aceito de propósito**: os passos de criar range chamam `setupNewRange`/`initTableConfig` de verdade — se o usuário estiver no meio de uma edição real não salva quando clica "Rever tutorial", esse progresso é sobrescrito. Isso espelha exatamente o que já acontece se a pessoa navegasse manualmente por esse mesmo fluxo (o app não tem aviso de "alterações não salvas" em lugar nenhum), então não é um risco novo introduzido pelo tour. `Pular tutorial`/clique no fundo/Esc encerram (`useModalA11y`), `Próximo` avança e no último passo vira `Concluir` — ambos `useStore.setState({ onboardingStep: null })` direto (sem action dedicada no store, fluxo efêmero). `onboardingStep: number | null` é resetado pra `null` em login/signup/logout/restoreSession, igual ao `justSignedUp`. Testado com RTL renderizando o `AppLayout` real (não só o componente isolado, já que ele dirige navegação de verdade) + um script Playwright ad-hoc que andou os 8 passos num Chromium real sem erro de console (mesmo padrão de `scripts/verify*.cjs`, descartado após a verificação).
