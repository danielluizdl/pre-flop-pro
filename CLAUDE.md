...
- **LoginPage** (login/signup/forgot + widget `Turnstile.tsx` se `VITE_TURNSTILE_KEY`), **WelcomeModal**, **OnboardingTour** (07/06, `src/components/Auth/OnboardingTour.tsx`), **ChangePasswordModal**. Ao fechar o `WelcomeModal`, em vez de só zerar `currentUser.firstLogin` (o que deixava o overlay montado-porém-invisível — `fixed inset-0` sem `justSignedUp` virar `false` mantinha o componente na árvore), agora zera `justSignedUp` **e** seta `onboardingStep: 0` no store, disparando o tour guiado só para quem acabou de criar conta.
  - **`OnboardingTour`, versão detalhada (07/07)**: navega de verdade por **13 passos em 6 páginas reais** (Dashboard → Meus Ranges → Configurar Mesa → Editor posição/nome/matriz → Configurar Mesa (roles/stack) + cenários → Drill selecionar + **mão ao vivo** → Range Check selecionar + **rodada ao vivo** → Histórico) chamando `setPage(...)` (e, nos passos de criar range, `setupNewRange(8,true,0.5)`/`initTableConfig()` — os mesmos actions que os botões reais "Continuar"/"Avançar" chamam, então é navegação honesta, não encenada). **Todo passo chama `setPage(...)` mesmo quando não navega de fato** (idempotente) — necessário porque "Voltar" pode vir de uma página diferente (ex.: de "Configurar Mesa" de volta pra "Editor"), então cada passo precisa garantir sozinho em qual página está, não pode depender de um passo anterior já ter navegado. Cada passo destaca um elemento real da página via atributo **`data-tour="<chave>"`** (`dashboard-hero`, `ranges-new`, `setup-tablesize`, `editor-position`, `editor-name`, `editor-matrix`, `table-editor-table`, `table-editor-scenarios`, `drill-select`, `drill-active`, `exercise-select`, `exercise-active`, `stats-header`, espalhados em `Dashboard.tsx`/`SituationsPage.tsx`/`RangeSetupPage.tsx`/`RangeEditorPage.tsx`/`TableEditorPage.tsx`/`TrainerPage.tsx`/`ExercisePage.tsx`/`StatsPage.tsx`), medido via `getBoundingClientRect` com **polling a cada 100ms por até 2.5s** (as páginas são lazy/Suspense — o alvo só existe depois do chunk carregar) — se não achar a tempo, cai num **fallback sem spotlight** (painel centralizado, tour não trava). O painel (título/descrição/contador "N/total") é **sempre visível e interativo desde o primeiro frame** (não espera medir pra aparecer) — só a posição anima do centro pro alvo quando medido; assim "Pular tutorial" fica sempre clicável. Botão **"Voltar"** (`stepIndex > 0`) decrementa `onboardingStep`, reaproveitando o mesmo `useEffect` (dependente de `stepIndex`) pra rodar o `run()` do passo anterior de novo. **Passos "ao vivo" (`drill-active`/`exercise-active`)**: em vez de só explicar o conceito, o tour **inicia de verdade** uma sessão de demonstração — `startDrillDemo()`/`startExerciseDemo()` escolhem um range real existente (`ranges.find(scenarios.length>0) ?? ranges[0]` pro drill; `ranges[0]` pro Range Check) e chamam `startDrillSession()+nextDrillHand()` / `startBuildSession()+confirmBuildSession()`, mostrando uma mão real respondível / uma rodada real pra pintar. **Nunca pisam numa sessão real em andamento**: só iniciam se `activeDrillRange`/`buildRounds` já não estiverem populados — se o usuário clicou "Rever tutorial" no meio de um treino de verdade, o tour só navega até lá e mostra a sessão real dele, sem reiniciar nada. Uma ref (`startedDrillDemo`/`startedBuildDemo`) marca quando a demo foi criada *pelo próprio tour*, e `finish()` (Concluir/Pular/Esc/clique fora) chama `stopDrill()`/`stopBuildSession()` só nesse caso — nunca encerra um treino real que já existia antes do tour. Nota fixa no rodapé do painel: *"Você pode reabrir este tutorial quando quiser em Perfil → Rever tutorial."* z-index 60 (acima do `TopNav`/modais, que usam 50). **Risco aceito de propósito**: os passos de criar range chamam `setupNewRange`/`initTableConfig` de verdade — se o usuário estiver no meio de uma edição real não salva quando clica "Rever tutorial", esse progresso é sobrescrito. Isso espelha exatamente o que já acontece se a pessoa navegasse manualmente por esse mesmo fluxo (o app não tem aviso de "alterações não salvas" em lugar nenhum), então não é um risco novo introduzido pelo tour. `onboardingStep: number | null` é resetado pra `null` em login/signup/logout/restoreSession, igual ao `justSignedUp`. Testado com RTL renderizando o `AppLayout` real (não só o componente isolado, já que ele dirige navegação de verdade), incl. os passos ao vivo do Drill/Range Check e a navegação Voltar cruzando página.
