...
- **Helpers** `functions/api/_utils.js`: `sha256Hex`, `hashPassword` (`SHA-256(salt + ':' + senha)`), `randomHex`, `getAuthUser`, `emailDomainExists` (DNS, fail-open), `checkRateLimit` (best-effort por `CF-Connecting-IP`, ~8/min), `verifyTurnstile` (siteverify; fail-open sem `TURNSTILE_SECRET_KEY`), `isHand`/`isUuidOrNull`/`isShortStr`, `json`, `CORS_HEADERS`, `handleOptions`.
...
- **Rotas** (`react-router-dom` + `src/components/Layout/RouterSync.tsx`): espelha URL↔`page` do store (`/dashboard`, `/ranges`, `/drill`, `/historico`, `/coach`); `public/_redirects` (`/* /index.html 200`). **Sentry** (`src/utils/sentry.ts`): init só com `VITE_SENTRY_DSN`; `ErrorBoundary` reporta. Lazy/Suspense em CoachPanel/TrainerPage/StatsPage.
- **LoginPage** (login/signup/forgot + widget `Turnstile.tsx` se `VITE_TURNSTILE_KEY`), **WelcomeModal**, **ChangePasswordModal**.
- **CoachPanel** (`src/components/Admin/CoachPanel.tsx`, rota `/coach`): abas "Visão do time" e "Por jogador". Filtros: `MultiPlayerSelect` (multi, checkboxes alfabéticos), select de range agrupado por posição, período. **Seção "Matriz do range"** em layout de **duas zonas** (`flex` sem wrap): esquerda `flex-1 min-w-0` com 3 painéis lado a lado — **Range real** (gabarito, do `range.grid` via `RangeActionGrid`), **Range jogado** (reconstruído de `played` via `RangeActionGrid`), **Precisão/erros** (`RangeHeatGrid`, métricas Precisão/Graves/Consultas/Volume); direita `shrink-0` com **`TopHandsPanel`** (abas Top 20 erros / Top 20 consultas, linhas clicáveis) + **`HandDetailCard`** (abre ao clicar uma mão). `ComboCounter` abaixo das matrizes real/jogado. Caixas **colapsáveis** (default minimizadas): **Resumo do time** (ORDENÁVEL clicando no cabeçalho ▲/▼ + linha clicável abre `PlayerQuickSummary` = ranges mais treinados / onde mais erra / mais consultados), Maiores leaks (impacto+Wilson), Hotspots, Por range (tipo de erro), Segmentos, Lacunas de conhecimento, Evolução (regressão), Leaks relativos (z-score). **"Foco da semana" foi REMOVIDO.**
...
### PENDÊNCIAS no painel coach (pedidas, NÃO implementadas ainda)
1. **Top 20 muito no canto direito**: aproximar do painel Precisão/erro; ao clicar uma mão, o layout "pula" pra esquerda — deixar o Top 20 fixo perto do grid e o `HandDetailCard` abrir sem deslocar o resto.
2. **Busca de range** (input pra digitar/filtrar) no filtro de range.
3. **Busca de jogadores** (input) no `MultiPlayerSelect`.
4. **Calendário/período custom**: opção "Custom" abrindo um seletor de datas (hoje só Tudo/7/30/90 dias).
