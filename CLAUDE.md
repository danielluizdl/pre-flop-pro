...
- **Replay de round no Histórico (`BuildRoundReplay` em `StatsPage.tsx`, 29/07)**: "Seu range"/"Gabarito" lado a lado (`flex flex-wrap gap-4`, `maxWidth={300}` cada — cabem os dois dentro do `max-w-2xl` da aba) em vez de empilhados; o `DiffGrid` (terceiro quadrado, "Diferença por mão") foi **removido** dessa visão pra ficar mais limpa — continua existindo no resultado ao vivo do round (`ExercisePage.tsx`, `BuildRound`/`BuildSummary`), só saiu do replay histórico.
