...
'pfp-build-history-v1'    → BuildSession[]   (histórico do modo Range Recall: rounds {label,score,attempt} + avgScore)
...
dashboard
  ...
  → exercise      (ExercisePage "Range Recall": BuildRangeSelect → BuildRound → BuildSummary)
  → history       (StatsPage)
```
...
## ExercisePage — modo "Range Recall" (nome escolhido; string central no i18n) (`src/components/Exercise/ExercisePage.tsx`, rota `/montar-range`, page `'exercise'`)
