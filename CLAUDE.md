...
src/
  components/
    ...
    Stats/        StatsPage.tsx, BuildAccountStats.tsx, AccuracySparkline.tsx
    Analysis/     AnalysisPage.tsx (página "Análise" pessoal — ver seção própria)
    Auth/         LoginPage.tsx, WelcomeModal.tsx, OnboardingTour.tsx, ChangePasswordModal.tsx, Turnstile.tsx,
                  AccountPage.tsx (dados da conta + trocar senha + Sessões ativas)
...
- **CoachPanel** (`src/components/Admin/CoachPanel.tsx`, rota `/coach`): três abas — **"Drill"** (ex-"Visão do time", `TeamView`), **"Range Check"** (`RecallView`) e **"Funcionalidades de Admin"** (`AdminView`). **A aba antiga "Por jogador" foi REMOVIDA** — **todas** as operações de conta (resetar senha, editar dados, excluir) ficam **só** na aba "Funcionalidades de Admin" (`ConfirmDangerModal`). O `PlayerQuickSummary` (expandir um jogador em "Resumo individual por jogador", aba Drill) é **somente leitura** — não tem mais botão de resetar senha (removido a pedido do Daniel: mexer em conta de usuário só deve existir num lugar).
  ...
  - **Vocabulário:** erro `grave` (interno, no D1/store/tipos) é exibido como **"Blunder"** em toda a UI (painel coach + app do jogador: feedback do drill, DrillSummary, `DrillCloudCards`/`StatsPage.tsx`, RangeHeatGrid); `severity:'grave'` NÃO muda no banco/código.
...
- **Progresso do jogador na conta** (10/07): `functions/api/me/stats.js` ganhou views `build-overview` (rounds, nota média/melhor, ranges, sessões, tempo, última atividade — tempo vem de `range_build_sessions`, resto de `range_build_events`; sessões = `COUNT(DISTINCT session_uuid)` com fallback), `build-by-range` e `build-sessions` (agrupa `range_build_events` por `session_uuid` + LEFT-merge da duração de `range_build_sessions`; sem a tabela nova as sessões saem com `durationSeconds: null`) — tudo `WHERE user_id` do token, fail-open por try/catch. UI: **`BuildAccountStats`** (`src/components/Stats/BuildAccountStats.tsx`, cards + Por range + Sessões recentes, padrão Skeleton/retry/vazio próprio) renderizado em 2 lugares: seção "Range Check (conta)" no topo da aba Range Check da `StatsPage` quando logado (histórico local abaixo ganha o título "Histórico neste dispositivo" e segue intacto p/ deslogados) e no topo da `AnalysisPage` (`hideByRangeTable`, sem a tabela "Por range" pra não duplicar a matriz por range que a própria `AnalysisPage` já mostra).
- **Cronômetro por round (29/07)**: `buildRoundStartMs` no store marca o início de cada round — setado em `confirmBuildSession` (início real do round 1, depois da tela de confirmação) e resetado em `nextBuildRound`/`retryBuildRound`. `BuildRound` mostra um `ElapsedClock` ao vivo (rótulo "Tempo") enquanto pinta; ao enviar, `submitBuildRound` calcula `durationSeconds = Date.now() - buildRoundStartMs` e grava no `BuildRoundResult` (novo campo obrigatório) — mostrado como chip ao lado da nota no resultado do round. `makeBuildSession` persiste `durationSeconds` por round em `BuildHistoryRound` e a soma dos rounds como `BuildSession.durationSeconds` (histórico local, `pfp-build-history-v1`); `BuildSummary` mostra um card "Tempo total" (soma) ao lado da nota média + duração por round na lista. `StatsPage` (`BuildHistoryPanel`) mostra a duração por round e o total da sessão tanto no histórico local quanto no da nuvem. Telemetria: `submitBuildRound` envia `durationSeconds` no evento `range-build`, persistido na coluna `duration_seconds` de `range_build_events` (**`schema_v10.sql` — migração D1 aplicada no remoto em 29/07**; endpoint com fail-open em cascata, igual ao padrão de `wrong_hands`/`user_grid`/`answer_grid`); `functions/api/me/stats.js` (`build-session-rounds`) devolve `durationSeconds` por round pro replay da nuvem, também fail-open se a coluna ainda não existir. Não mexe no total de sessão já existente (`range_build_sessions.duration_seconds`, tempo de parede via `build-session-end`) — os dois convivem: total local = soma dos rounds, total cloud (`build-sessions`/`BuildAccountStats`) = tempo de parede real da sessão.
- **Replay de round no Histórico (`BuildRoundReplay` em `StatsPage.tsx`, 29/07)**: "Seu range"/"Gabarito" lado a lado (`flex flex-wrap gap-4`) em vez de empilhados; o `DiffGrid` (terceiro quadrado, "Diferença por mão") foi **removido** dessa visão pra ficar mais limpa — continua existindo no resultado ao vivo do round (`ExercisePage.tsx`, `BuildRound`/`BuildSummary`), só saiu do replay histórico. **Ajustes de tamanho/expansão múltipla (29/07)**: grids em `maxWidth={475}` (300 → 380 → +25% a pedido do Daniel, "os combos ficavam pequenos pra visualizar direitinho"); a aba "Range Check" da `StatsPage` usa um container mais largo que as outras abas (`max-w-6xl` vs `max-w-2xl`) só pra caber os dois lado a lado confortavelmente nesse tamanho. `openRounds` é um `Set<string>` de `roundKey` (era `string | null`) — **múltiplos rounds da mesma sessão podem ficar expandidos ao mesmo tempo** (clicar um não fecha o outro); cada replay aberto tem um botão **X** próprio (`onClose`) pra fechar só aquele, além do clique na linha continuar alternando via `toggleRound`.

## AnalysisPage — página "Análise" pessoal (`src/components/Analysis/AnalysisPage.tsx`, rota `/analise`, page `'analysis'`)
- Item próprio da navegação principal (TopNav/Sidebar, entre Range Check e Histórico, ícone BarChart3), lazy no `AppLayout`. Era a aba "Análise" do Histórico (`MyCoachPanel`, removido) — promovida a página em 11/07.
- Versão pessoal da aba Drill do painel do coach, escopada só ao usuário logado: filtros Período/Range (reusa `PeriodFilter`/`RangeSelect` de `CoachPanel/shared`), matriz Range real vs jogado (`RangeActionGrid` + `ComboSummary`), Precisão/erros (`RangeHeatGrid`), `TopHandsPanel` + `HandDetailCard` em slot fixo `w-[270px]` (sem reflow), tabelas ordenáveis Por range / Maiores leaks / Consulta no drill (com `ConsultRangeDetail`). SEM `MultiPlayerSelect` — nunca dados de outros jogadores.
- Dados do endpoint player-scoped `functions/api/me/analytics.js` (whitelist `PLAYER_VIEWS`, `playerIds` SEMPRE do token — regra de segurança inegociável).
- Deslogado (defensivo — na prática userMode null cai na LoginPage): estado vazio pedindo login, sem chamar a API.
- Headings: h1 (título) → h2 (Matriz do range) → h3 (Precisão/erros e `Section`) — axe `heading-order` exige o incremento de 1 em 1.

## StatsPage (`src/components/Stats/StatsPage.tsx`)
- Abas: "Histórico de Sessões", "Range Check", "Desempenho Global em Drill". A antiga aba "Análise" virou a página própria acima. **A aba "Meus dados na nuvem" foi REMOVIDA (29/07)** — `MyAccountStats.tsx` foi deletado e seu conteúdo redistribuído pro destino lógico de cada pedaço (Daniel achou que a aba "não fazia sentido" como algo à parte):
  - Os 7 cards agregados (Mãos/Precisão/Blunders/Imprecisos/Consultas/Sessões/Tempo treinado, `functions/api/me/stats?view=overview`) viraram o componente `DrillCloudCards` (interno a `StatsPage.tsx`) e agora aparecem no topo da aba **"Desempenho Global em Drill"** (só quando `currentUser`, acima do acordeão por posição/heatmap local — daí o nome da aba ganhar "em Drill": os cards são scope de conta/nuvem, o resto do painel é local/`handPerformance`).
  - As tabelas "Por range" e "Piores mãos" (que liam `view=by-range`/`view=by-hand`) foram **removidas sem substituto** — o Daniel considerou redundante com o que "Desempenho Global em Drill" já mostra localmente. Os views `by-range`/`by-hand` continuam existindo em `functions/api/me/stats.js` (não foram apagados, só ficaram sem consumidor no front — mesmo padrão de utils "testados mas não usados" do projeto).
  - **`BuildAccountStats`** (cards+tabelas do Range Check) migrou pra dentro da aba **"Range Check"** da própria `StatsPage`, acima do `BuildHistoryPanel` (eyebrow `t.myAccount.buildTitle` = "Range Check (conta)"), só quando `currentUser`. Continua sendo reutilizado também no topo da `AnalysisPage` (`hideByRangeTable`).
  - **`DevicesSection`** ("Sessões ativas", gerenciar/derrubar sessões logadas em outros aparelhos) migrou pra **`AccountPage.tsx`** (`src/components/Auth/AccountPage.tsx`, rota alcançada via menu de perfil do `TopNav` → "Editar conta"), como uma terceira caixa depois de "Dados da conta" e "Trocar senha".
