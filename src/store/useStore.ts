...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      authSignup: async (username, password, inviteCode, name, email, tier, turma, turnstileToken) => {
        try {
          const res = await fetch('/api/auth/signup', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username, password, inviteCode, name, email, tier, turma, turnstileToken: turnstileToken ?? undefined }),
          })
          const data = await res.json().catch(() => null)
          if (!res.ok || !data) return { ok: false, error: data?.error ?? t.netErrors.server(res.status) }
          sessionStorage.setItem('pfp-auth-token', data.token)
